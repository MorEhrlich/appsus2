...
const KEY = 'gglBooksDB'

// window.books = gGoogleBooks;
...
function getById(bookId) {
  return Promise.resolve(query().find(book => book.id === bookId ));
}

function addReview(bookId, review) {

  let book = query().find(book => book.id === bookId )
  if (!book.reviews) {
    book.reviews=[];
  }
...
function getGoogleBooks(searchText) {

  let books = loadGoogleBooks(searchText)

  if (!books) books = queryForGoogleBooks(searchText)

  return books

}

function queryForGoogleBooks(searchText) {

  const url =
  `https://www.googleapis.com/books/v1/volumes?printType=books&q=${searchText}`;

  return axios.get(url).then( booksData => { return booksData.data.items } )
                      .then(items => 
                      { return items.map(item => convertGoogleBookToBook(item))})
                      .then(books => saveGoogleBooks(searchText, books))  
}

function convertGoogleBookToBook(googleBook) {
  ...
  book.authors = googleBook.volumeInfo.authors; // should be here [...authors] ?
  ...
  function loadGoogleBooks(searchText) {

    return storageService.load(`${KEY},${searchText}`);
  }

  function saveGoogleBooks(searchText, books) {

    storageService.save(`${KEY},${searchText}`, books)
  }
